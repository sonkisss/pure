...
#### ✅ 修复发票上传因 OSS 返回 403 导致的“文件内容验证错误”

- 影响范围：销售发票列表/进项发票列表/新增发票弹窗的上传流程。
- 问题：上传后远程 fetch 校验签名 URL，OSS 私有桶返回 403 或 application/xml，导致本地提示“文件内容验证错误，请重新上传”。
- 处理：移除远程内容校验，改为本地 PDF 校验（基于文件类型/扩展名），避免依赖 OSS 返回头；上传成功后仍写入路径与名称。
- 风险与验证：仅校验 PDF 类型；已在本地上传 PDF 通过，未再出现 403 误报。

#### ✅ 修复发票预览签名链接覆盖 content-type 导致 OSS 403 / 强制下载

- 影响范围：点击发票预览跳转新窗口。
- 问题：签名 URL 强制 `response-content-type=application/pdf`，OSS 返回 `InvalidRequest Can not override response header on content-type`，浏览器控制台报错并显示 XML。
- 处理：签名时仅通过 `response-content-disposition=inline` 设置内联，不覆盖 content-type；上传时显式写入文件的 Content-Type/Content-Disposition=inline，保证浏览器识别并内联预览；解析旧链接时去掉 query 重新签名，避免历史参数导致下载；新增 `VITE_OSS_PUBLIC_READ`，公共读时直接使用 public URL（不再追加 response 参数，避免匿名用户覆盖响应头报错）；若签名失败继续回退公共 URL。
- 验证：更新后签名链接应可直接预览，不再出现 OSS 403/XML 提示。

#### ✅ 修复删除发票文件未同步删除 OSS 对象

- 影响范围：发票编辑弹窗内“删除发票文件”。
- 问题：仅清空数据库字段与前端状态，OSS 实际文件未删除。
- 处理：删除前提取对象路径并调用 OSS 删除接口，删除成功后再清空数据库字段与 UI。

#### ✅ 删除发票记录时同步删除 OSS 文件

- 影响范围：销售/进项发票“删除”操作。
- 问题：删除记录仅清空数据库，OSS 文件残留。
- 处理：删除记录前先尝试删除 OSS 文件，失败则中止并提示。

#### ✅ 统一其他模块 OSS 文件删除行为

- 影响范围：合同附件删除、客户挂账 PDF 删除、询价删除、供应商付款凭证删除。
- 问题：部分模块仅删数据库记录或状态，OSS 文件未清理。
- 处理：合同附件删除前先删除 OSS 文件；客户挂账记录/客户删除时删除关联 PDF；删除询价时清理附件文件；供应商付款记录/供应商删除时清理凭证文件。

#### ✅ 统一费用模块 OSS 附件删除行为

- 影响范围：费用删除、批量删除。
- 问题：删除费用记录未同步清理 OSS 附件，导致对象残留。
- 处理：删除费用前先读取附件列表并逐个删除 OSS 对象，成功后再删除记录；批量删除同样处理。
