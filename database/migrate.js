...
async function migrateSuppliers() {
  try {
    ...
    const supplierContent = fs.readFileSync(supplierMockPath, 'utf8');
...
async function generateSupplierDebts() {
  try {
    ...
    // 获取供应商列表
    const supplierResult = await client.query('SELECT id FROM suppliers LIMIT 30');
...
async function generateCustomerPayments() {
  try {
    ...
    const customerResult = await client.query('SELECT id FROM customers LIMIT 50');
...
async function migrate() {
  try {
    ...
    const { supplier_count, customer_count, product_count, debt_count, payment_count } = stats.rows[0];
    log('数据统计:', 'info');
